...
{
    List<Vector2> points;
    public List<Vector2> Points { get => points; private set => points = value; }

    [Inject]
    public LineConstructer(Settings s)
	{
        points = ConstructLine(s.minAngle, s.maxAngle, s.minLength, s.maxLength, Vector3.zero, Vector2.up * s.minimalPlaneHeight, new List<Vector2>());
	}
